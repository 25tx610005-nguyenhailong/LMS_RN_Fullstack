...
function Notifications() {
  ...
  //fetch data invitation new
  const dispatch = useDispatch()
  useEffect(() => {
    // dispatch(fetchInvitationsAPI())
    //function handle real-time
    //https://socket.io/how-to/use-with-react
    const onReceiveNewInvitation = (invitation) => {
      if (invitation.inviteeId === currentUser._id) {
        //step1: Add new invitation in redux
        dispatch(addNotification(invitation))
        //step2: Update status has notification
            ...
            >
              ...
              >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box>
                    <strong>{notification.inviter?.displayName}</strong> đã mời bạn tham gia bảng{' '}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'flex-end' }}>
                  {notification.boardInvitation.status === BOARD_INVITATION_STATUS.ACCEPTED && (
                    <Chip icon={<DoneIcon />} label='Đã đồng ý' color='success' size='small' />
                  )}
